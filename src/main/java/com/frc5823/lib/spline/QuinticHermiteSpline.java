package com.frc5823.lib.spline;
...
public class QuinticHermiteSpline extends Spline {
   ...
   * @param p0 The starting pose of the spline
   * @param p1 The ending pose of the spline
   * @param dp0 The starting velocity of the spline
   * @param dp1 The ending velocity of the spline
   */
  public QuinticHermiteSpline(Pose2d p0, Pose2d p1, Twist2d dp0, Twist2d dp1) {
    this(p0, p1, dp0, dp1, ReferenceFrame.NONE);
  }
   ...
   * @param ref ReferenceFrame of this spline
   */
  public QuinticHermiteSpline(Pose2d p0, Pose2d p1, Twist2d dp0, Twist2d dp1, ReferenceFrame ref) {
    // double scale = 1.2 * p0.getTranslation().distance(p1.getTranslation());
    // Rotation2d direction = p0.inverse().transformBy(p1).getRotation();
    x0 = p0.getTranslation().x();
    x1 = p1.getTranslation().x();
    dx0 = dp0.dx * Math.cos(dp0.dtheta);
    dx1 = dp1.dx * Math.cos(dp1.dtheta);
    ddx0 = 0;
    ddx1 = 0;
    y0 = p0.getTranslation().y();
    y1 = p1.getTranslation().y();
    dy0 = dp0.dy * Math.sin(dp0.dtheta);
    dy1 = dp1.dy * Math.sin(dp1.dtheta);
    ddy0 = 0;
    ddy1 = 0;
  ...
  private static void runOptimizationIteration(List<QuinticHermiteSpline> splines) {
    ...
    double stepSize =
        fitParabola(p1, p2, p3); // approximate step size to minimize sumDCurvature2 along the
    // gradient
