...
/**
 * This class represents a vector field that moves around a given polygon. Along the sides, the
 * field is parallel to the side. Around the vertices, the field is rotational. Intended use is for
 * obstacle avoidance.
 *
 * @author Joseph Reed
 */
public class PolyCirculationVectorField extends VectorField {
  ...
  public PolyCirculationVectorField(List<Translation2d> vertices_, double radius_) {
    ...
    vertices.add(
        vertices_.get(
            1)); // add second last so we can uhhh like use three points in our corner checking
    // code
  }
  ...
  public Translation2d getVector(Translation2d here) {
    ...
    // 2. Find the region containing here. Return a vector parallel to its defining
    // side.
    ...
    for (int i = 1; i < vertices.size() - 1; i++) {
      ...
      if (here.isWithinAngle(
          vertices.get(i - 1), vertices.get(i), vertices.get(i + 1), true // check within
          // vertical angle
          )) {
        if (here_.norm() <= radius) {
          return (new Translation2d(-here_.y(), here_.x()))
              .normalize(); // use translated position because it
          // will rotate about the vertex
        } else {
    ...
    for (int i = 0; i < vertices.size() - 1; i++) {
      if (here.isWithinAngle(vertices.get(i), center, vertices.get(i + 1))) {
