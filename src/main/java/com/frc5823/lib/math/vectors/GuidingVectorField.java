...
public class GuidingVectorField extends VectorField {
   ...
   * Function<Translation2d,Double> dfdx, Function<Translation2d,Double> dfdy) {
   * phi = surface; dfdx_ = dfdx; dfdy_ = dfdy; } public
   * GuidingVectorField(Function<Translation2d,Double> surface,
   * Function<Translation2d,Double> dfdx, Function<Translation2d,Double> dfdy,
   * boolean isReversed) { phi = surface; dfdx_ = dfdx; dfdy_ = dfdy;
   * if(isReversed) direction = -1; }
   */
  public GuidingVectorField(
      BaseSurface surface_, boolean isReversed, Function<Translation2d, Double> k_) {
    surface = surface_;
    if (isReversed) direction = -1;
    k = k_;
  }

  public GuidingVectorField(BaseSurface surface_) {
    this(surface_, false, here -> 1.0);
  }

  public GuidingVectorField(BaseSurface surface_, boolean isReversed) {
    this(surface_, isReversed, here -> 1.0);
  }

  public GuidingVectorField(BaseSurface surface_, Function<Translation2d, Double> k_) {
    this(surface_, false, k_);
  }

  protected int direction = 1;
  /*
   * protected Function<Translation2d,Double> phi; protected
   * Function<Translation2d,Double> dfdx_; protected
   * Function<Translation2d,Double> dfdy_;
   */
