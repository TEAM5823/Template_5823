...
public class GuidingPolynomialVectorField extends VectorField {
   ...
   * setDfc(fc); if(heading.getRadians() >= Math.PI) direction = -1;
   * }
   */
  public GuidingPolynomialVectorField(
      double[] coeffs, boolean isReversed, Function<Translation2d, Double> k_) {
    fc = coeffs;
    setDfc(fc);
